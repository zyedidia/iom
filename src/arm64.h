...
static inline bool
arm64_is_adr(cs_insn insn)
{
    return insn.id == ARM64_INS_ADR;
}

static inline int64_t
arm64_target(cs_insn insn)
{
...
arm64_reassemble(cs_insn insn, uint32_t data, int64_t imm)
{
    switch (insn.id) {
    case ARM64_INS_B:
        imm = imm >> 2;
...
static inline uint32_t
arm64_bl(uint32_t imm)
{
    return 0x94000000 | imm;
}

static inline uint32_t
arm64_b(uint32_t imm)
{
    return 0x14000000 | imm;
}

static inline uint32_t
arm64_adr_imm(uint32_t insn)
{
